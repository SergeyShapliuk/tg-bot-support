...
class Navigation {
    ...
    private async addAndShowScreen(screen: AppScreen) {
        ...
        // Добавляем контейнер навигации на stage, если он еще не добавлен
        if (!this.container.parent) {
            app.stage.addChild(this.container);
        }

        // Добавляем экран на контейнер
        this.container.addChild(screen);

        // Подготовка экрана, если доступно
        if (screen.prepare) {
            screen.prepare();
        }

        // Обработка изменения размера экрана, если доступно
        if (screen.resize) {
            // Триггерим первое изменение размера
            screen.resize(this.width, this.height);
        }

        // Добавляем функцию обновления экрана, если доступно
        if (screen.update) {
        ...
        try {
            if (screen.show) {
                screen.interactiveChildren = false; // Отключаем взаимодействие, пока экран показывается
                await screen.show(); // Дожидаемся завершения показа экрана
                screen.interactiveChildren = true; // Включаем взаимодействие
            }
        } catch (error) {
