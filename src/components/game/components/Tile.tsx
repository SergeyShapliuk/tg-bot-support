...
function useRunDevOnlyTestsForTile({index, ...tileProps}: TileProps) {
    ...
    try {
        ...
        if (error instanceof Error) {
            console.error(`Warning at Tile ${index}`, error.message, error.message);
        }
...
function testTilePosition({size, position}: Pick<TileProps, "position" | "size">) {
    if (size.x === 100 && size.y === 100) {
        if (position.x !== 0 || position.z !== 0) {
            throw new Error(
                `Tile size and position mismatch. Expected tile position to be 0 when size is 100. Actual position: ${JSON.stringify(position)}`
            );
